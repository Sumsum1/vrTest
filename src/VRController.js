...
THREE.VRController = function(gamepad) {
  ...
  this.dof = gamepad.pose
    ? 3 * (+gamepad.pose.hasOrientation + +gamepad.pose.hasPosition)
    : 0;
  ...
  this.inspect = function() {
    return (
      ...
      "\n\n\tVibration intensity: " +
      this.vibeChannels.intensity +
      "\n\tVibration channels:" +
      this.vibeChannels.reduce(function(a, e) {
        return (
          a +
          '\n\t\tName: "' +
          e.name +
  ...
  this.pollForChanges = function() {
    ...
    if (axesNames.length > 0) {
      axesNames.forEach(function(axesName) {
        ...
        if (axesChanged) {
          ...
          if (controller.style === "vive" && axesName === "thumbpad")
            axesValues[1] *= -1;

          if (verbosity >= 0.7)
            console.log(
              controllerInfo + axesName + " axes changed",
              axesValues
            );
    ...
    //  Otherwise we need to check and update ALL values.
      ...
      if (axesChanged) {
        if (verbosity >= 0.7)
          console.log(controllerInfo + "axes changed", axes);
    ...
    buttons.forEach(function(button, i) {
      ...
      if (button.value !== gamepad.buttons[i].value) {
        ...
        if (verbosity >= 0.6)
          console.log(controllerAndButtonInfo + "value changed", button.value);
        controller.dispatchEvent({
      ...
      if (button.isTouched !== gamepad.buttons[i].touched) {
        ...
        if (verbosity >= 0.5)
          console.log(controllerAndButtonInfo + "touch " + eventAction);
        controller.dispatchEvent({
          type: button.name + " touch " + eventAction
        });
        if (isPrimary)
          controller.dispatchEvent({ type: "primary touch " + eventAction });
      }
      ...
      if (button.isPressed !== gamepad.buttons[i].pressed) {
        ...
        if (verbosity >= 0.5)
          console.log(controllerAndButtonInfo + "press " + eventAction);
        controller.dispatchEvent({
          type: button.name + " press " + eventAction
        });
        if (isPrimary)
          controller.dispatchEvent({ type: "primary press " + eventAction });
      }
    });
  };
};
...
THREE.VRController.prototype.update = function() {
  ...
  //  do the following math.
    ...
    if (this.armModel === undefined) {
      if (THREE.VRController.verbosity >= 0.5)
        console.log(
          "> #" +
            gamepad.index +
            " " +
...
THREE.VRController.prototype.applyVibes = function() {
  if (this.gamepad.hapticActuators && this.gamepad.hapticActuators[0]) {
    ...
    if (
      renderedIntensity !== this.vibeChannels.prior ||
      now - this.vibeChannels.lastCommanded >
        THREE.VRController.VIBE_TIME_MAX / 2
    ) {
      this.vibeChannels.lastCommanded = now;
      this.gamepad.hapticActuators[0].pulse(
        renderedIntensity,
        THREE.VRController.VIBE_TIME_MAX
      );
...
THREE.VRController.onGamepadConnect = function(gamepad) {
  ...
  if (scope.verbosity >= 0.5)
    console.log("vr controller connected", controller);
  if (scope.verbosity >= 0.7) console.log(controller.inspect());
  window.setTimeout(function() {
    window.dispatchEvent(
      new CustomEvent("vr controller connected", { detail: controller })
    );
  }, 500);
};
THREE.VRController.onGamepadDisconnect = function(gamepad) {
  ...
  if (scope.verbosity >= 0.5)
    console.log("vr controller disconnected", controller);
...
THREE.VRController.update = function() {
  ...
  for (i = 0; i < gamepads.length; i++) {
    ...
    gamepad = gamepads[i];
    if (
      gamepad !== undefined && //  Just for you, Microsoft Edge!
      ...
      else if (this.controllers[i] !== undefined)
...
OrientationArmModel.prototype.update = function() {
  ...
  // tool tips.
  var controllerEuler = new THREE.Euler().setFromQuaternion(
    this.controllerQ,
    "YXZ"
  );
  ...
  var elbowOffset = new THREE.Vector3().copy(
    OrientationArmModel.ARM_EXTENSION_OFFSET
  );
  ...
  var lerpValue =
    lerpSuppression *
    (elbowRatio +
      wristRatio * extensionRatio * OrientationArmModel.EXTENSION_RATIO_WEIGHT);
...
1:      elbow_rot * (kControllerForearm + (wrist_rot * kControllerPosition))
  ...
  var offset = new THREE.Vector3().copy(
    OrientationArmModel.ARM_EXTENSION_OFFSET
  );
